...
async def is_authenticated(request: Request, is_valid: bool = Depends(is_valid_token)):
    if not is_valid:
        return templates.TemplateResponse("login.html", {"request": request})


@app.get("/admin", response_model=str)
async def admin_page(request: Request, is_valid: bool = Depends(is_authenticated)):
    """html admin
    """
...
@app.get("/dialogs")
async def dialogs_page(request: Request, is_valid: bool = Depends(is_authenticated)):
    """
    Отображение страницы входа.
    """
...
@app.get("/profile")
async def profile_page(request: Request, is_valid: bool = Depends(is_authenticated)):
    """
    Отображение страницы входа.
    """
