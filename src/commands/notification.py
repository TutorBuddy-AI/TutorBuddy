...
async def send_pin_message(bot,chat_id,speaker,message_history):
    ...
    #if message_history == 13:
    if message_history:
        markup_translate = types.InlineKeyboardMarkup(
            inline_keyboard=[[types.InlineKeyboardButton('📖 Translate', callback_data='pin_message_translate')]]
        )
        ...
        await bot.send_photo(chat_id, photo=types.InputFile(file_path_img), caption=get_pin_message(translate=False), reply_markup=markup_translate)
